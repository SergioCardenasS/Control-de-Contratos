...
class ingenieria_window(QWidget):
	def __init__(self):
		super(ingenieria_window, self).__init__()
		...
		self.setWindowTitle('Ingenieria')
		self.show()
	...
	def Finalizar(self):
		...
		if index.isValid():
			ventana = temporary.ventanaFinalizar(id_contract=self.listaContratos[index.row()].id_contract,ID_AREA=AREA_INGENIERIA_ID).exec_()
