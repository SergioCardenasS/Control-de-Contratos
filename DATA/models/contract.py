...
class Contract:
	...
	def insert(self,cursor_db):
		...
		insert_code_contract="""INSERT INTO Contract
								...
								values('%s','%s','%d',CONVERT(bit,'%d'),'%s','%s','%d','%s'
								)"""%(self.purchase_order,
	...
	def update(self,cursor_db):
		if(str_is_invalid(self.purchase_order) or str_is_invalid(self.contract_number) or str_is_invalid(self.special_contract)):
			return False
		update_code_contract="""UPDATE Contract SET purchase_order='%s', contract_number='%s', id_process='%d', contract_type=CONVERT(bit,'%d'), mod_date='%s', iteration_number='%d', special_contract='%s'
							WHERE id_contract='%d'"""%(
									self.purchase_order,
									self.contract_number,
									self.id_process,
