...
class FinishIngenieriaAvios(QDialog):
	def __init__(self, s_avios ,parent=None):
		super(FinishIngenieriaAvios, self).__init__(parent)
		self.avios=s_avios
		db=get_connection()
	...
	def pantalla(self):
		...
		self.OtherBoton = QPushButton("Regresar a Desarrollo", self)
		...
		self.connect(self.crearBoton, SIGNAL("clicked()"), self.FinishIngenieriaAviosTask)
		self.connect(self.OtherBoton, SIGNAL("clicked()"), self.FinishToDesarrollo)

	def FinishIngenieriaAviosTask(self):
		...
		if reply == QMessageBox.Yes:
			...
			if(controller_avios.avios_is_equal(db,self.avios)):
				...
				if(self.avios.update(db.cursor())):
					new_comment = comment_avios.CommentAvios([self.avios.id_avios,controller_comment_avios.get_next_number_comment_by_id_avios(db,self.avios.id_avios),AREA_INGENIERIA_ID,Commentary,self.avios.mod_date])
	...
	def FinishToDesarrollo(self):
		reply=QMessageBox.question(self,
		'Message',"Enviar a Desarrollo el Control de Avios del contrato: "+str(self.contract.contract_number),
		QMessageBox.Yes,QMessageBox.No)
		if reply == QMessageBox.Yes:
			Commentary = unicode(self.editCommentary.toPlainText())
			db=get_connection()
