...
class AviosLLegadas(QDialog):
	def __init__(self, s_avios ,parent=None):
		...
		self.setWindowTitle('Enviar Comentario')
		self.show()

	def pantalla(self):
		#Nombre de los campos
		#Creacion de botones
		self.crearBoton = QPushButton("Enviar a Logistica", self)
	...
	def SendAviosTask(self):
		...
		'Message',"Comunicar a Logistica sobre el avio del contrato: "+str(self.contract.contract_number),
		QMessageBox.Yes,QMessageBox.No)
		if reply == QMessageBox.Yes:
			Commentary = unicode(self.editCommentary.toPlainText())
			db=get_connection()
			if(controller_avios.avios_is_equal(db,self.avios)):
				new_comment = comment_avios.CommentAvios([self.avios.id_avios,controller_comment_avios.get_next_number_comment_by_id_avios(db,self.avios.id_avios),AREA_CALIDAD_ID,Commentary,get_time_str()])
...
class FinishCalidadAvios(QDialog):
	def __init__(self, s_avios ,parent=None):
		super(FinishCalidadAvios, self).__init__(parent)
		self.avios=s_avios
		db=get_connection()
		self.contract=controller_contract.get_contracts_by_id(db,self.avios.id_contract)
		db.close()
		...
		left=(desktopSize.width()/2)
		self.resize(left, top)
		self.setWindowTitle('Finalizar Avios')
		self.show()
	...
	def pantalla(self):
		...
		self.OtherBoton = QPushButton("Regresar a Logistica", self)
		self.atrasBoton = QPushButton("Atras", self)
		#Creando el grid
		#Creacion de la tabla  con cada item
		self.tabla = QTableWidget()
		...
		self.atrasBoton.setFixedSize(150, 110)
		self.crearBoton.setFixedSize(150, 110)
		self.OtherBoton.setFixedSize(150, 110)
		...
		grid.addWidget(self.editCommentary,6,3)
		grid.addWidget(self.crearBoton,6,4)
		grid.addWidget(self.OtherBoton,6,1)
		self.setLayout(grid)
		#Funcionalidades de los botones
		self.atrasBoton.clicked.connect(self.close)
		self.connect(self.crearBoton, SIGNAL("clicked()"), self.FinishCalidadAviosTask)
		self.connect(self.OtherBoton, SIGNAL("clicked()"), self.FinishToLogistica)

	def FinishCalidadAviosTask(self):
		reply=QMessageBox.question(self,
		'Message',"Aceptar y Finalizar el Control de Avios del contrato: "+str(self.contract.contract_number),
		QMessageBox.Yes,QMessageBox.No)
		if reply == QMessageBox.Yes:
			Commentary = unicode(self.editCommentary.toPlainText())
			db=get_connection()
			if(controller_avios.avios_is_equal(db,self.avios)):
				...
				self.avios.id_process=PROCESS_AVIOS_COMPLETED_ID
				if(self.avios.update(db.cursor())):
					new_comment = comment_avios.CommentAvios([self.avios.id_avios,controller_comment_avios.get_next_number_comment_by_id_avios(db,self.avios.id_avios),AREA_CALIDAD_ID,Commentary,self.avios.mod_date])
					if(new_comment.insert(db.cursor())):
						db.commit()
						db.close()
						self.close()
	...
	def FinishToLogistica(self):
		reply=QMessageBox.question(self,
		'Message',"Enviar a Logistica el Control de Avios del contrato: "+str(self.contract.contract_number)+" para seguir con las llegadas de avios",
		QMessageBox.Yes,QMessageBox.No)
		if reply == QMessageBox.Yes:
			Commentary = unicode(self.editCommentary.toPlainText())
			db=get_connection()
