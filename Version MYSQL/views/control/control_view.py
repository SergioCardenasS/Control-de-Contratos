...
class ventanaBusqueda(QDialog):
	def __init__(self, parent=None):
		...
		CClient = QLabel('Cliente')
		self.ediClient = QLineEdit()
		optionlabel = QLabel('Buscar por:')
		self.editOption = QComboBox()
		self.editOption.addItem("PO")
		self.editOption.addItem("Contrato")
		self.editOption.addItem("Cliente")
		#Creando el grid
		grid = QGridLayout()
		...
		grid.addWidget(CClient,3,0)
		grid.addWidget(self.ediClient,3,1)
		grid.addWidget(optionlabel,4,0)
		grid.addWidget(self.editOption,4,1)

		grid.addWidget(self.aceptarBoton,5,1)
		grid.addWidget(self.cancelarBoton,5,2)
	...
	def Crear(self):
		...
		self.client = self.ediClient.text()
		optiontext = str(self.editOption.currentText())
		if(optiontext=="PO"):
			self.option = 1
		elif(optiontext=="Cliente"):
			self.option = 3
		elif(optiontext=="Contrato"):
			self.option = 2
		if(self.option==1):
			...
			if(str_is_invalid(self.client)):
				QMessageBox.warning(self, 'Error',"Codigo de Cliente No Valido", QMessageBox.Ok)
			else:
				self.close()
		elif(self.option==2):
			if(is_invalid_contract_number(self.contract_number)):
