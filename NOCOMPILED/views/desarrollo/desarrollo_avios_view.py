...
from views.desarrollo import desarrollo_avios_fin_process
from views.control import control_view_dialog
from views.control import control_avios_view_dialog

class desarrollo_window(QDialog):
	def __init__(self,parent=None):
		super(desarrollo_window, self).__init__(parent)
		...
		self.setWindowTitle(TITLE_APP+AVIOS_TITLE+DESARROLLO_TITLE)
		self.show()
		...
		for index in range(len(self.listaAvios)):
			...
			if(actual_contract):
				...
				if(self.listaAvios[index].id_process==PROCESS_AVIOS_FIN_DES_ID):
					if (time_pass_one_day(str(self.listaAvios[index].mod_date),9)):
	...
	def Finalizar(self):
		if(self.control_singleton):
			...
			if index.isValid():
				if(self.listaAvios[index.row()].id_process==PROCESS_AVIOS_ACTIVATE_ID):
					ventana = desarrollo_avios_fin_process.StartAviosControl(s_avios=self.listaAvios[index.row()]).exec_()
				elif(self.listaAvios[index.row()].id_process==PROCESS_AVIOS_FIN_DES_ID):
					ventana = desarrollo_avios_fin_process.FinishDesarrolloAvios(s_avios=self.listaAvios[index.row()]).exec_()
				self.refresh_table(AREA_DESARROLLO_ID)
			self.control_singleton=False
